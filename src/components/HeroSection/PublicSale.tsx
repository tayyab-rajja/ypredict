...
const LIMIT_TIME_REMAINING = "2023-01-28T00:00:00Z";
...
const CountDown = (props: { timeRemaining }) => {
  ...
  return (
    (time?.day >= 0 && time?.hour >= 0 && time?.min >= 0 && time?.sec >= 0) ? (
      <div className="flex flex-row gap-[10px]">
        ...
        <div className="flex flex-col justify-center items-center rounded-md w-[60px] px-2 py-2">
          ...
          <span className="text-[10px]">{time?.min > 1 ? 'Minutes' : 'Minute'}</span>
        </div>
        <div className="flex flex-col justify-center items-center rounded-md w-[60px] px-2 py-2">
          <span className="text-[20px]">{time?.sec}</span>
          <span className="text-[10px]">{time?.sec > 1 ? 'Seconds' : 'Second'}</span>
        </div>
      </div>
    ) : (
      <span
      ...
      >
        {time?.day > 0 && time?.hour > 0 && time?.min > 0 && time?.sec > 0 && ("Finished")}
      </span>
    )
  );
};
...
export default function PublicSale() {
  ...
  useEffect(() => {
    ...
    const getBlockTime = async () => {
      ...
      const endDate_sec = (await contract.endDate()).toString();
      const timeRemaining = endDate_sec - blockTimestamp;

      var cd_total = (Number(new Date(LIMIT_TIME_REMAINING)) - Number(new Date())) / 1000;

      if (cd_total < 0) {
        cd_total = 1;
      }

      setTimeRemaining(cd_total);
